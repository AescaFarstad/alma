...
function parseArgs() {
  ...
  if (!inputArg || !outputArg || !generatedTypesFileArg) {
    console.error('Usage: node filter_geojson.cjs --input=<directory> --output=<directory> --generated-types-file=<file>');
    process.exit(1);
  }
...
function getInscribedCenter(geometry) {
  let points = [];
  if (geometry.type === 'Polygon') {
    points = geometry.coordinates.flat(1);
  } else if (geometry.type === 'MultiPolygon') {
    points = geometry.coordinates.flat(2);
  }
...
function processFeature(feature, log) {
...
function isLineStringClosed(coordinates) {
  const first = coordinates[0];
  const last = coordinates[coordinates.length - 1];
  return first[0] === last[0] && first[1] === last[1];
}

function processBuilding(feature, log) {
  const props = feature.properties;

  // 2.1.1 `building` Tag Refinement - Removal rules
  if (!['Polygon', 'MultiPolygon'].includes(feature.geometry.type)) {
     if(feature.geometry.type === 'LineString' && isLineStringClosed(feature.geometry.coordinates)) {
  ...
  for (const category in buildingCategoryMap) {
    if (buildingCategoryMap[category].includes(props.building)) {
...
function processHighway(feature, log) {
  const props = feature.properties;
  ...
  if (removeHighways.includes(props.highway)) {
  ...
  for (const category in highwayCategoryMap) {
    if (highwayCategoryMap[category].includes(props.highway)) {
...
function printLog(log, fileName) {
...
function main() {
  ...
  for (const fileName of files) {
    ...
    if (log.totals.buildings === 0) delete log.totals.buildings;
    if (log.totals.highways === 0) delete log.totals.highways;
  ...
  try {
    const generateScriptPath = path.join(__dirname, 'generate-structure.cjs');
