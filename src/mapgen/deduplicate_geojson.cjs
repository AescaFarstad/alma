/*
    ...
    7.  Logging & Output:
        ...
        - Logs a comprehensive report for each file, including:
            ...
            - The total number of vertices remaining in the final geometry.
...
const turf_bbox = require('@turf/bbox').default;
...
function getVertexRing(geometry) {
...
function areRingsCongruent(ringA, ringB, tolerance) {
...
function areFeaturesCongruent(featureA, featureB, tolerance) {
...
function getTriangleCentroids(poly) {
    const centroids = [];
...
function checkCentroids(centroids, polyToCheckA, polyToCheckB) {
...
async function deduplicateGeojson({ inputDir, outputDir }) {
    try {
        ...
        for (const file of files) {
            ...
            const geojson = fs.readJsonSync(sourcePath);
            ...
            tree.load(features.map((feature, index) => {
                const [minX, minY, maxX, maxY] = turf_bbox(feature);
            ...
            for (let i = 0; i < features.length; i++) {
                ...
                const [minX, minY, maxX, maxY] = turf_bbox(featureA);
